...
class Vocabulary(object):
    ...
    def __init__(self, init_from_file=None, max_length=140):
        ...
        self.special_tokens = ['EOS', 'GO']  # need rewrite it
        ...
        self.voc_file = init_from_file
        #if init_from_file: self.init_from_file(init_from_file)
    def update_attri_values(self):
        self.additional_chars = set()
        self.chars = self.special_tokens
        self.vocab_size = len(self.chars)
        self.vocab = dict(zip(self.chars, range(len(self.chars))))
        self.reversed_vocab = {v: k for k, v in self.vocab.items()}
        self.init_from_file(self.voc_file)
    ...
    def add_characters(self, chars):
        ...
        self.vocab_size = len(self.chars)
        self.vocab = dict(zip(self.chars, range(len(self.chars))))  #zcc: dict{ frag:index}
        self.reversed_vocab = {v: k for k, v in self.vocab.items()}  # zcc: dict{index:frag}
...
class MolData(Dataset):
    ...
    def __init__(self, data, voc):
        ...
        con = data.columns.values[1:]
        self.con = data[con].values.tolist()
    ...
    #  have done it in 'generate_prior_model*.py' script.
    '''
