...
class decoderTransformer(nn.Module):
    def __init__(self, vocab_size, d_model, nhead, num_decoder_layers, dim_feedforward, max_seq_length,
        ...
        self.embed_tgt = nn.Embedding(vocab_size, d_model)  # zcc: d_model: the dim of verb
...
class transformer_RL():
    ...
    def likelihood(self, target,len_con):
        """
        ...
        '''
        print('original_target',target[0])
        print(target.size())
        print('con_token',con_token)
        print('x',x[0])
        print(x.size())
        print('y_target',y_target[0])
        print(y_target.size())
        exit()
        '''
        # 去掉token长度
    ...
    def generate(self, batch_size, max_length=140, con_token_list= ['is_JNK3', 'is_GSK3', 'high_QED', 'good_SA']):
       ...
       """
        ...
        for step in range(max_length):
            # print(step)
            logits = sample_forward_model(self.decodertf, input_vector, con_tokens,self.device)
...
def Prior_train_forward(model, tgt, con_token,device):
    tgt = torch.tensor(tgt)

    tgt_mask = gen_nopeek_mask(tgt.shape[1],device)
...
def sample_forward_model(model, tgt, con_token,device):
...
def gen_nopeek_mask(length,device):
